...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints ):
        ...
        self.nassigns = 0
        self.backtracks = 0     # Initialize the backtrack counter
        self.nodes_visited = 0  # Initialize the nodes visited counter

        self.weights = dict()  # for dom_wdeg
        for var1 in self.variables:
            for var2 in self.variables:
                self.weights[(var1, var2)] = 1
...
def domwdeg(assignment, csp):
    ...
    for var in csp.variables:
        ...
        if csp.curr_domains:
            ...
            value = float(dom / wdeg)
        else:
            value = count(csp.nconflicts(var, val, assignment) == 0 for val in csp.domains[var])
